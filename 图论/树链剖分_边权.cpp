...
// 树剖用
struct ed{int v,w,id,next;}edge[2*maxn];
...
namespace tree{
    ...
    inline void add_edge(int u,int v,int id,int w) {
        edge[++cnt]=ed{v,w,id,head[u]};
        head[u]=cnt;
    }
    ...
    inline void dfs1(int cur,int fath,int he){ //dfs(root,0,1)
        ...
        for(int i=head[cur];i;i=edge[i].next){
            if(edge[i].v!=fath){
                ...
                a[edge[i].v]=edge[i].w;      //边权放到儿子身上
                if(siz[edge[i].v]>siz[son[cur]]) son[cur]=edge[i].v;
                if(u[edge[i].id]!=cur) swap(u[edge[i].id],v[edge[i].id]); //交换两点，方便点权更新
            }
        }
    }
    ...
    */
    inline int query_sum(int x,int y){
        ...
        if(min(id[x],id[y])<max(id[x],id[y])) ans=(ans+segment_tree::query_sum(1,1,n,min(id[x],id[y])+1,max(id[x],id[y])));
        return ans;
    }
    ...
    inline int query_max(int x,int y){
        ...
        if(min(id[x],id[y])<max(id[x],id[y])) ans=max(ans,segment_tree::query_max(1,1,n,min(id[x],id[y])+1,max(id[x],id[y])));
        return ans;
    }
    //单点修改
    ...
    inline void modify(int x,int y,int add){
        ...
        if(min(id[x],id[y])<max(id[x],id[y])) segment_tree::modify(1,1,n,min(id[x],id[y])+1,max(id[x],id[y]),add);
    }
}
...
int main()
{
    ...
    while(t--) {
        ...
        for(int i=1;i<n;i++){
            ...
            add_edge(u[i],v[i],i,w[i]);
            add_edge(v[i],u[i],i,w[i]);
        } 
        build();
        while(1) {
            scanf("%s",opt+1);
